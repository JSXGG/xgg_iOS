...
@property(nonatomic,strong)userItem *MainUseritem;
...
- (void)viewDidLoad {
    ...
    self.MainUseritem = [userItem getuserItemWithuid:self.uid];
    ...
    for (int i = 0; i<[Array count]; i++){
        addUserItem *item = [[addUserItem alloc]init];
        item.Title = Array[i];
        if (i == 1) {
...
-(void)ClickOnthefinish{
    if ([self.listItems count]==4) {
        addUserItem *item1 = self.listItems[0];
        addUserItem *item2 = self.listItems[1];
        addUserItem *item3 = self.listItems[2];
        addUserItem *item4 = self.listItems[3];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0){
        ...
        [relationVC GetrelationSelect:^(relationItem *item){
            addUserItem *UserItem = WeakSelf.listItems[0];
            UserItem.Content = item.relation_ch;
            UserItem.relationText = item.relation_en;
            
            addUserItem *fistnameItem = WeakSelf.listItems[1];
        ...
        addUserItem *UserItem = WeakSelf.listItems[3];
...
-(NSString *)GetrecomandfirstName:(NSString *)relation{
    ...
    //如果是女生,添加儿子或者女二的话，需要获取丈夫的姓。
        ...
        NSArray *listItem = [userItem findByCriteria:[NSString stringWithFormat:@"where Mainuid = '%@'",self.uid]];
        for (userItem *userItem in listItem){
